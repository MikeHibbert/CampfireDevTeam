...
import { CamperResponse, ErrorResponse } from '../types/partyBox';
...
export class CommandHandler {
    ...
    constructor(
        ...
        this.fileOpsManager = new FileOperationsManager(workspaceManager);
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                try {
                    ...
                    const partyBox = await this.partyBoxManager.createPartyBoxPayload(
                        'generate_code',
                        taskDescription,
                        attachments
                    );
     ...
     */
    async handleReviewCode(): Promise<void> {
        try {
            ...
            if (document.isDirty) {
                const saveFirst = await vscode.window.showWarningMessage(
                    'The current file has unsaved changes. Save before review?',
                    'Save and Review',
                    'Review Anyway',
                    'Cancel'
                );
     ...
     */
    private async handleCampfireResponse(
        response: CamperResponse | ErrorResponse, 
            ...
            default:
                ...
                if (camperResponse.content) {
                    vscode.window.showInformationMessage(
                        `${operationType} result: ${camperResponse.content.substring(0, 100)}...`
                    );
                }
        }
        ...
        if (camperResponse.confidence_score < 0.7) {
            vscode.window.showWarningMessage(
                `Note: ${camperResponse.camper_role} has low confidence (${Math.round(camperResponse.confidence_score * 100)}%) in this response`
            );
        }
    }
     ...
     */
    private async handleCodeResponse(response: CamperResponse): Promise<void> {
     ...
     */
    private async handleSuggestionResponse(response: CamperResponse): Promise<void> {
        // Show suggestions in information message with option to view details
        const action = await vscode.window.showInformationMessage(
            `${response.camper_role}: ${response.content.substring(0, 100)}...`,
            'View Details',
            'Dismiss'
        );
     ...
     */
    private async handleCommandResponse(response: CamperResponse): Promise<void> {
        if (response.commands_to_execute.length === 0) {
            return;
        }

        // Ask user for permission to execute commands
        const executeCommands = await vscode.window.showWarningMessage(
            `${response.camper_role} suggests executing ${response.commands_to_execute.length} command(s). Execute them?`,
            'Execute All',
            'Review First',
            'Cancel'
        );
