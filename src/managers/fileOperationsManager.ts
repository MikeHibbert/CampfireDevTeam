...
export class FileOperationsManager {
    ...
    private readonly supportedFileTypes = new Map<string, FileTypeInfo>([
        ['.py', { 
            type: 'python', 
            defaultDir: 'src', 
    ...
    async createCodeFile(
        ...
        try {
            ...
            if (!this.workspaceManager.isPathWithinWorkspace(fullPath)) {
                const securityError = this.errorHandler.handleSecurityError(
                    'File path is outside workspace boundary',
                    { fileName, suggestedLocation, workspaceRoot }
                );
                return {
                    success: false,
                    filePath: fullPath,
            ...
            const fileError = this.errorHandler.handleFileOperationError(
                error,
                'create file',
                fileName
            );
            ...
            if (fileError.severity === ErrorSeverity.HIGH || fileError.severity === ErrorSeverity.CRITICAL) {
        ...
        for (const file of files) {
            ...
            if (!result.success && options.overwriteConfirmation !== false) {
                ...
                vscode.window.showErrorMessage(
                    `Failed to create ${file.path}: ${result.error}`
                );
            }
        }
...
/**
    ...
    private async createBackup(filePath: string): Promise<string> {
        ...
        try {
            ...
            const backupError = this.errorHandler.handleFileOperationError(
                error,
                'create backup',
                filePath
            );
     ...
     */
    private determineFileLocation(fileName: string): string {
     ...
     */
    async getProjectStructureInfo(): Promise<ProjectStructureInfo> {
        const workspaceRoot = this.workspaceManager.getWorkspaceRoot();
        if (!workspaceRoot) {
            return {
                directories: [],
     ...
     */
    private async scanDirectories(rootPath: string, maxDepth: number = 3): Promise<string[]> {
     ...
     */
    private async categorizeFilesByType(rootPath: string): Promise<Record<string, string[]>> {
        const filesByType: Record<string, string[]> = {};

        const scanFiles = async (currentPath: string): Promise<void> => {
            try {
                const entries = await fs.readdir(currentPath, { withFileTypes: true });
     ...
     */
    private suggestDefaultLocation(directories: string[], filesByType: Record<string, string[]>): string {
        ...
        const hasSourceFiles = Object.keys(filesByType).some(type => 
            ['python', 'javascript', 'typescript'].includes(type)
        );
  ...
  /**
     ...
     */
    getSupportedFileTypes(): Map<string, FileTypeInfo> {
     ...
     */
    getFileTypeInfo(fileName: string): FileTypeInfo | undefined {
        const extension = path.extname(fileName).toLowerCase();
     ...
     */
    suggestFileName(content: string, fileType?: string): string {
     ...
     */
    private extractNamesFromContent(content: string, fileType?: string): string[] {
        const names: string[] = [];
        ...
        if (fileType === 'python' || content.includes('def ') || content.includes('class ')) {
            ...
            if (classMatches) {
                names.push(...classMatches.map(match => 
                    match.replace('class ', '').toLowerCase()
                ));
            }

            const functionMatches = content.match(/def\s+(\w+)/g);
            if (functionMatches) {
                names.push(...functionMatches.map(match => 
                    match.replace('def ', '').toLowerCase()
                ));
            }
        }
        ...
        if (fileType === 'javascript' || fileType === 'typescript' || 
            ...
            if (functionMatches) {
                names.push(...functionMatches.map(match => 
                    match.replace('function ', '').toLowerCase()
                ));
            }
        }
     ...
     */
    validateFileName(fileName: string): { isValid: boolean; suggestions: string[]; errors: string[] } {
        const errors: string[] = [];
     ...
     */
    async getFileCreationStats(): Promise<{
        totalFiles: number;
     ...
     */
    private async getRecentFiles(rootPath: string): Promise<string[]> {
        const recentFiles: string[] = [];
        const oneDayAgo = Date.now() - (24 * 60 * 60 * 1000);

        const scanForRecent = async (currentPath: string): Promise<void> => {
            try {
                const entries = await fs.readdir(currentPath, { withFileTypes: true });
...
export class AdvancedFileOperations {
    ...
    constructor(fileOpsManager: FileOperationsManager) {
     ...
     * Detect project type based on files and structure
     */
    private async detectProjectType(workspaceRoot: string, structureInfo: ProjectStructureInfo): Promise<ProjectType> {
        try {
     ...
     */
    private getDirectoryConventions(projectType: ProjectType): DirectoryConventions {
        const conventions: Record<ProjectType, DirectoryConventions> = {
            'typescript-web': {
                source: ['src', 'lib'],
     ...
     */
    private analyzeStructureCompliance(
        structureInfo: ProjectStructureInfo, 
        conventions: DirectoryConventions
    ): StructureCompliance {
     ...
     */
    private findMissingDirectories(
        existingDirs: string[], 
        conventions: DirectoryConventions
    ): string[] {
     ...
     */
    async getOptimalFileLocation(fileName: string, fileType?: string): Promise<string> {
     ...
     */
    private categorizeFile(fileName: string, fileType?: string, fileTypeInfo?: FileTypeInfo): FileCategory {
     ...
     */
    private selectTargetDirectory(category: FileCategory, conventions: DirectoryConventions): string {
        switch (category) {
            case 'test':
     ...
     */
    private validateAndAdjustDirectory(targetDir: string, analysis: ProjectAnalysis): string {
        const workspaceRoot = this.fileOpsManager['workspaceManager'].getWorkspaceRoot();
        if (!workspaceRoot) {
            return '.';
        }
     ...
     */
    private getDefaultProjectAnalysis(): ProjectAnalysis {
        return {
            projectType: 'generic',
...
// Type definitions for advanced file operations
...
type FileCategory = 'source' | 'test' | 'config' | 'docs' | 'assets';

interface DirectoryConventions {
    source: string[];
    tests: string[];
    assets: string[];
    config: string[];
