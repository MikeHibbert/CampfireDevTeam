...
import { BackendRequest, ErrorResponse } from '../types/partyBox';
...
export interface CodeSectionResult {
    success: boolean;
    filePath: string;
    content: string;
    startLine?: number;
...
export interface CodeUpdateRequest extends BackendRequest {
    action: 'update_code';
    parameters: {
        path: string;
        content: string;
...
export class BackendRequestHandler {
    ...
    private requestHistory: BackendRequest[] = [];
     ...
     */
    public async handleBackendRequest(request: BackendRequest): Promise<any> {
        try {
            // Validate request
     ...
     */
    private async handleDirectoryListingRequest(request: BackendRequest): Promise<DirectoryListingResult> {
        try {
     ...
     */
    private async readDirectoryContents(absolutePath: string, relativePath: string): Promise<DirectoryEntry[]> {
     ...
     */
    private async handleConsoleOutputRequest(request: BackendRequest): Promise<ConsoleOutputResult> {
        try {
     ...
     */
    private async handleCodeSectionRequest(request: BackendRequest): Promise<CodeSectionResult> {
        try {
            const targetPath = request.target_path || request.parameters?.path;
     ...
     */
    private validateRequest(request: BackendRequest): RequestValidationResult {
        const errors: string[] = [];
     ...
     */
    private addToRequestHistory(request: BackendRequest): void {
     ...
     */
    public getRequestHistory(): BackendRequest[] {
     ...
     */
    private async handleCodeUpdateRequest(request: BackendRequest): Promise<CodeUpdateResult> {
        try {
            const updateRequest = request as CodeUpdateRequest;
     ...
     */
    private async createNewFile(resolvedPath: string, targetPath: string, content: string): Promise<CodeUpdateResult> {
        try {
     ...
     */
    public createConfirmationResponse(
        action: string, 
        success: boolean, 
        details: any = {}
    ): { confirmation: { action: string; success: boolean; timestamp: string; details: any } } {
        return {
            confirmation: {
                action,
                success,
     ...
     */
    public createRequestFailureResponse(
        action: string, 
        reason: string, 
        details: any = {}
    ): ErrorResponse {
