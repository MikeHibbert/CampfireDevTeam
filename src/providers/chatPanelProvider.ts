...
export class CampfireChatPanelProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'campfire.chatPanel';
    ...
    private determineClaimFromMessage(text: string): 'generate_code' | 'review_code' | 'execute_command' {
    ...
    private showFileCreationOptions(files: Array<{path: string, content: string}>): void {
    ...
    private showCommandExecutionOptions(commands: string[]): void {
        if (!this._view) return;
    ...
    private selectCamper(camper: string): void {
    ...
    private async handleFileCreation(file: {path: string, content: string}): Promise<void> {
        try {
    ...
    public updateConfiguration(config: CampfireConfig): void {
        this.config = config;
        this.partyBoxManager = new PartyBoxManager(config);
        this.mcpClient = new MCPClient(config);
    }
