...
class MCPClient {
    constructor(config) {
        ...
        this.circuitBreaker = new errorHandler_1.CircuitBreaker(5, 60000, 300000); // 5 failures, 1 min recovery, 5 min monitoring
    }
    /**
     ...
     */
    async sendTorchRequest(partyBox) {
        try {
            // Validate Party Box before sending
            const validation = this.partyBoxManager.validatePartyBoxPayload(partyBox);
            if (!validation.isValid) {
                const validationError = this.errorHandler.handleValidationError(validation.errors, 'Party Box validation');
                return {
                    error: {
            ...
            return await this.circuitBreaker.execute(async () => {
                return await errorHandler_1.RetryManager.withRetry(() => this.sendHttpRequest(mcpRequest), {
                    ...
                    onRetry: (attempt, error) => {
                        ...
                        vscode.window.showInformationMessage(`Retrying connection to CampfireValley (attempt ${attempt}/${this.maxRetries})...`);
                    }
     ...
     */
    async sendHttpRequest(mcpRequest) {
        return new Promise((resolve, reject) => {
            try {
                // Validate MCP server URL
                let serverUrl;
                try {
                    serverUrl = new url_1.URL(this.config.mcpServer);
                }
                catch (error) {
                    const configError = this.errorHandler.handleConfigurationError('mcpServer', `Invalid URL format: ${this.config.mcpServer}`);
                ...
                try {
                    ...
                    if (requestSize > maxRequestSize) {
                        const sizeError = this.errorHandler.createError(errorHandler_1.ErrorType.VALIDATION, 'REQUEST_TOO_LARGE', `Request size ${requestSize} bytes exceeds maximum ${maxRequestSize} bytes`);
                        reject(new Error(sizeError.technicalMessage));
                        return;
                    }
                }
                catch (error) {
                    const serializationError = this.errorHandler.createError(errorHandler_1.ErrorType.VALIDATION, 'REQUEST_SERIALIZATION_FAILED', 'Failed to serialize request data');
                    ...
                    res.on('end', () => {
                        ...
                        try {
                            ...
                            if (!res.statusCode || res.statusCode < 200 || res.statusCode >= 300) {
                                const httpError = this.errorHandler.handleNetworkError(new Error(`HTTP ${res.statusCode}: ${res.statusMessage}`), 'HTTP response error');
                            ...
                            try {
                                ...
                                const jsonError = this.errorHandler.createError(errorHandler_1.ErrorType.NETWORK, 'RESPONSE_PARSE_ERROR', 'Failed to parse JSON response from server');
                                reject(new Error(`${jsonError.technicalMessage}: ${parseError}`));
                                return;
                            }
                            // Validate response structure
                            if (typeof parsedResponse !== 'object' || parsedResponse === null) {
                                const structureError = this.errorHandler.createError(errorHandler_1.ErrorType.VALIDATION, 'INVALID_RESPONSE_STRUCTURE', 'Server returned invalid response structure');
                            ...
                            const processingError = this.errorHandler.createError(errorHandler_1.ErrorType.NETWORK, 'RESPONSE_PROCESSING_ERROR', 'Error processing server response');
                ...
                req.on('error', (error) => {
                    ...
                    const networkError = this.errorHandler.handleNetworkError(new Error(`${errorMessage}: ${error.message}`), 'HTTP request error');
                    reject(new Error(networkError.technicalMessage));
                });
                req.on('timeout', () => {
                    req.destroy();
                    const timeoutError = this.errorHandler.createError(errorHandler_1.ErrorType.TIMEOUT, 'REQUEST_TIMEOUT', `Request timeout after ${this.timeout}ms`);
                    reject(new Error(timeoutError.technicalMessage));
                });
                // Handle request abortion
                req.on('abort', () => {
                    const abortError = this.errorHandler.createError(errorHandler_1.ErrorType.NETWORK, 'REQUEST_ABORTED', 'Request was aborted');
                ...
                try {
                    ...
                    const sendError = this.errorHandler.createError(errorHandler_1.ErrorType.NETWORK, 'REQUEST_SEND_ERROR', 'Failed to send request data');
                    reject(new Error(`${sendError.technicalMessage}: ${writeError}`));
                }
            }
            catch (error) {
                const unexpectedError = this.errorHandler.createError(errorHandler_1.ErrorType.UNKNOWN, 'UNEXPECTED_REQUEST_ERROR', 'Unexpected error during request preparation');
        ...
        try {
            ...
            return {
                ...
                diagnostics: {
                    ...
                    camper_role: response.camper_role,
    ...
    async testBasicConnectivity() {
        try {
            const serverUrl = new url_1.URL(this.config.mcpServer);
            ...
            return new Promise((resolve) => {
                ...
                req.on('error', (error) => {
                    resolve({
                        ...
                        diagnostics: { error_code: error.code, error_message: error.message }
                    });
                });
                req.on('timeout', () => {
                    req.destroy();
