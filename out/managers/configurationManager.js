...
class ConfigurationManager {
    ...
    loadConfiguration() {
        ...
        const config = {
            ...
            enableAutoCompletion: vsCodeConfig.get('enableAutoCompletion', config_1.DEFAULT_CONFIG.enableAutoCompletion),
            responseTimeout: vsCodeConfig.get('responseTimeout', config_1.DEFAULT_CONFIG.responseTimeout),
            retryAttempts: vsCodeConfig.get('retryAttempts', config_1.DEFAULT_CONFIG.retryAttempts),
            logLevel: vsCodeConfig.get('logLevel', config_1.DEFAULT_CONFIG.logLevel),
            securityValidation: vsCodeConfig.get('securityValidation', config_1.DEFAULT_CONFIG.securityValidation),
            workspaceValidation: vsCodeConfig.get('workspaceValidation', config_1.DEFAULT_CONFIG.workspaceValidation),
            confirmFileOverwrites: vsCodeConfig.get('confirmFileOverwrites', config_1.DEFAULT_CONFIG.confirmFileOverwrites),
            maxFileSize: vsCodeConfig.get('maxFileSize', config_1.DEFAULT_CONFIG.maxFileSize)
        };
        ...
        if (validation.warnings && validation.warnings.length > 0) {
            vscode.window.showWarningMessage(`Campfire configuration warnings: ${validation.warnings.join(', ')}`);
        }
        return config;
    }
    reloadConfiguration() {
    ...
    async updateConfiguration(key, value) {
        try {
            ...
            if (!validation.isValid) {
                vscode.window.showErrorMessage(`Configuration update resulted in invalid state: ${validation.errors.join(', ')}`);
                return false;
            }
    ...
    extractFieldFromError(errorMessage) {
        if (errorMessage.includes('MCP server'))
            return 'mcpServer';
        if (errorMessage.includes('Party Box'))
            return 'partyBoxPath';
        ...
        if (errorMessage.includes('log level'))
            return 'logLevel';
    ...
    async resetToDefaults() {
        try {
            ...
            for (const [key, value] of Object.entries(config_1.DEFAULT_CONFIG)) {
    ...
    async importConfiguration(configJson) {
        try {
            ...
            for (const [key, value] of Object.entries(importedConfig)) {
                if (key in config_1.DEFAULT_CONFIG && key !== 'workspaceRoot' && key !== 'osType') {
                    await vsCodeConfig.update(key, value, vscode.ConfigurationTarget.Workspace);
                }
            }
            this.reloadConfiguration();
            ...
            vscode.window.showErrorMessage(`Failed to import configuration: ${error instanceof Error ? error.message : 'Invalid JSON'}`);
            return false;
        }
    }
    /**
